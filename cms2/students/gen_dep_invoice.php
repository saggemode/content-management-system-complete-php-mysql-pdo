            ...
            <div class="row">
                ...
                <div class="col-md-9">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="col-sm-6">
                                <h3 class="text-primary float-left">Generate Departmental Fee Invoice </h3>
                            </div>
                            ...
                            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                                ...
                                if ($check_query->rowCount() > 0) {
                                    ...
                                    $insert_query = $con->prepare('INSERT INTO department_invoice (date,name, session,level, payment, program, regid, department_fee, invoice_no,confirmation_code)
                                                                              VALUES      
                                    ...
                                    header('location: department_invoice.php');
                                }
                                ...
                                <div class="col-sm-6">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" name="payment_type" id="payment_type">
                                            <?php
                                            $query = $con->prepare('SELECT * FROM departmental_fee ORDER BY id DESC');
                                            $query->execute();
                                            if ($query->rowCount() > 0) {
                                                while ($row = $query->fetch()) {
                                                    $cat_year = $row['amount'];
                                                    echo "<option value='" . $cat_year . "' " . ((isset($departmental_fee) and $departmental_fee === $cat_year) ? 'selected' : '') . '>' . ucfirst($cat_year) . '</option>';
                                                }
                                            } else {
                                                echo '<center><h6>No Session Available</h6></center>';
                                            }
