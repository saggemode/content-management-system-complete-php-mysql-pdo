            ...
            <div class="row">
                ...
                <div class="col-md-9">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="col-sm-6">
                                <h3 class="text-primary float-left">Generate T Shirt Fee Invoice </h3>
                            </div>
                            ...
                            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                                ...
                                $invoice = substr(str_shuffle($chars), 0, 8);
                                $chars_confirm_code = '0123456789';
                                $confirmation_code = substr(str_shuffle($chars_confirm_code), 0, 8);


                                $check_query = $con->prepare('SELECT * FROM department_invoice WHERE invoice_no = ?');
                                ...
                                if ($check_query->rowCount() > 0) {
                                    ...
                                    $insert_query = $con->prepare('INSERT INTO shirt_invoice (date,name, session,level, payment, program, regid, shirt_fee, invoice_no,confirmation_code)
                                                                              VALUES      
                                    ...
                                    header('location: tshirt_invoice.php');
                            ...
                            <form action="" method="POST" role="form" enctype="multipart/form-data">
                                ...
                                <div class="col-sm-6">
                                    <div class="form-group">
                                        <label for="payment_type" class="text-primary">Payment Amount</label>
                                        <select class="form-control" name="payment_type" id="payment_type">
                                            <?php
                                            $query = $con->prepare('SELECT * FROM shirt_fee ORDER BY id DESC');
                                            $query->execute();
                                            if ($query->rowCount() > 0) {
                                                while ($row = $query->fetch()) {
                                                    $cat_year = $row['amount'];
                                                    echo "<option value='" . $cat_year . "' " . ((isset($shirt_fee) and $shirt_fee === $cat_year) ? 'selected' : '') . '>' . ucfirst($cat_year) . '</option>';
                                                }
                                            } else {
                                                echo '<center><h6>No Session Available</h6></center>';
                                            }
