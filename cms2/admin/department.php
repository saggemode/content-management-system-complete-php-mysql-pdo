...
if (isset($_GET['del'])) {
    ...
    if ($_SESSION['role'] == 'admin') {
        $del_check_query = $con->prepare('SELECT * FROM department_invoice WHERE id = :zid');
    ...
    if ($del_check_query->rowCount() > 0) {
        $del_query = $con->prepare('DELETE FROM department_invoice WHERE id = :zid');
...
if (isset($_POST['checkboxes'])) {
    ...
    foreach ($_POST['checkboxes'] as $user_id) {
        ...
        if ($bulk_option === 'delete') {
            $bulk_delete_query = $con->prepare('DELETE FROM department_invoice WHERE id = ? ');
...
<div id="wrapper">
    ...
    <div class="container-fluid body-section">
        <div class="row">
            ...
            <div class="col-md-9">
                <h1 class="text-primary"><i class="mdi mdi-office-building"></i> department Payment
                    <small class="text-secondary">view all department payment</small>
                </h1>
                <hr>
                <nav aria-label="breadcrumb" role="navigation">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="#"><i class="fa fa-tachometer"></i> Dashboard</a></li>
                        <li class="breadcrumb-item active" aria-current="page"><i class="fa fa-building"></i> department payment</li>
                    </ol>
                </nav>
                ...
                <?php
                if ($_SESSION['role'] === 'admin') {
                    $stmt = $con->prepare('SELECT * FROM department_invoice ORDER BY id DESC');
                    $stmt->execute();
                }
                if ($stmt->rowCount() > 0) {
                ?>
                <form action="" method="post">
                    ...
                    <table class="table table-bordered text-center table-responsive-md" id="example" width="100%" cellspacing="0">
                        ...
                        <tr>
                            ...
                            <td>
                                <a href=""><span class="badge badge-success">Receipt</span></a>
                    ...
                    <?php
                        ...
                        echo "<h2 class='text-center'>No department invoice Available</h2>";
                    }
                    ?>
                </form>
