...
if (isset($_GET['del'])) {
    ...
    if ($_SESSION['role'] == 'admin') {
        $del_check_query = $con->prepare('SELECT * FROM shirt_invoice WHERE id = :zid');
    ...
    if ($del_check_query->rowCount() > 0) {
        $del_query = $con->prepare('DELETE FROM shirt_invoice WHERE id = :zid');
        $del_query->execute(array('zid' => $del_id));
        if ($del_query->execute()) {
            $msg = 'invoice Has Been Deleted';
            header('refresh:2; url=shirt.php');
        } else {
...
if (isset($_POST['checkboxes'])) {
    ...
    foreach ($_POST['checkboxes'] as $user_id) {
        ...
        if ($bulk_option === 'delete') {
            $bulk_delete_query = $con->prepare('DELETE FROM shirt_invoice WHERE id = ? ');
...
<div id="wrapper">
    ...
    <div class="container-fluid body-section">
        <div class="row">
            ...
            <div class="col-md-9">
                <h1 class="text-primary"><i class="fa fa-shirtsinbulk"></i> Tshirt Payment
                    <small class="text-secondary">view all shirt payment</small>
                </h1>
                <hr>
                <nav aria-label="breadcrumb" role="navigation">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="#"><i class="fa fa-tachometer"></i> Dashboard</a></li>
                        <li class="breadcrumb-item active" aria-current="page"><i class="fa fa-shirtsinbulk"></i> shirt payment</li>
                    </ol>
                </nav>
                ...
                <?php
                if ($_SESSION['role'] === 'admin') {
                    $stmt = $con->prepare('SELECT * FROM shirt_invoice ORDER BY id DESC');
                    $stmt->execute();
                }
                if ($stmt->rowCount() > 0) {
                ?>
