...
if (isset($_GET['del'])) {
    ...
    if (isset($_SESSION['username']) && $_SESSION['role'] === 'admin') {
        $del_query = $con->prepare('DELETE FROM departmental_fee WHERE id = ?');
        $del_query->execute(array($del_id));
        if ($del_query->execute()) {
            $del_msg = 'Amount Has Been Deleted';
            header('refresh:2; url=departmental_fee.php');
        } else {
            $del_error = 'Amount Has not Been Deleted';
            header('refresh:2; url=departmental_fee.php');
        }
    }
}
...
if (isset($_POST['submit'])) {
    ...
    if (empty($cat_name)) {
        ...
        $check_query = $con->prepare('SELECT * FROM departmental_fee  WHERE amount = ?');
        $check_query->execute(array($cat_name));
        if ($check_query->rowCount() > 0) {
            $error = 'Amount Already Exist';

        } else {
            $insert_query = $con->prepare("INSERT INTO departmental_fee (amount) VALUES ('$cat_name')");
//            $insert_query->execute(array($cat_name));
            if ($insert_query->execute()) {
                $msg = 'Amount Has Been Added';
                header('refresh:2; url=departmental_fee.php');
            } else {
                $error = 'Amount Has not Been Added';
                header('refresh:2; url=departmental_fee.php');
            }
        }
    }
...
if (isset($_POST['update'])) {
    ...
    if (empty($cat_name)) {
        ...
        $check_query->execute(array($cat_name));
        if ($check_query->rowCount() > 0) {
            $up_error = 'Amount Already Exist';
        } else {
            $update_query = $con->prepare('UPDATE departmental_fee SET amount = ? WHERE id = ? ');
            $update_query->execute(array($cat_name,$edit_id));
            if ($update_query->execute()) {
                $up_msg = ' Amount Has Been Updated';
                header('refresh:2; url=departmental_fee.php');
            } else {
                $up_error = ' amount Has not Been Updated';
                header('refresh:2; url=departmental_fee.php');
            }
        }
    }
            ...
            <div class="col-md-9">
                ...
                <div class="row">
                    <div class="col-md-4 mb-4">
                        ...
                        <?php
                        if (isset($_GET['edit'])) {
                            $edit_check_query = $con->prepare('SELECT * FROM departmental_fee WHERE id = ?');
                    ...
                    <div class="col-md-8 mb-4 "><br>
                        <?php
                        $get_query = $con->prepare('SELECT * FROM departmental_fee ORDER BY id DESC');
                        $get_query->execute();
                            ...
                            <table id="example"
                                ...
                                while ($get_row = $get_query->fetch()) {
                                    ...
                                    <tr>
                                        ...
                                        <td><a href="departmental_fee.php?edit=<?php echo $id; ?>"><i
                                                    class="btn  btn-info btn-md waves-effect waves-light  btn-sm fa fa-edit"></i></a>
                                        </td>
                                        <td><a href="departmental_fee.php?del=<?php echo $id; ?>"><i
